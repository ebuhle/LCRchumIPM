...
User inputs needed to fit the Lower Columbia chum IPM include, first, a data frame containing observations and ancillary information organized by population and year. We can look at the subset of rows of `fish_data_SMS` for the Crazy Johnson Creek population (coded `Grays CJ`) to see what column names and formats **salmonIPM** expects:
...
We can now call the function `salmonIPM()` to fit models to the historical Lower Columbia chum monitoring data. Here we use the Ricker spawner-recruit function:
