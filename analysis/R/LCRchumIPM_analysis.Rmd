...
$$ \widehat{M}_{jt} = f( S_{jt} | \alpha_{jt}, M_{\text{max},j} ) $$

Intrinsic productivity is calculated as a weighted mean of age-specific female fecundity, weighted by the spawner age distribution, multiplied by the proportion female spawners and the population-specific density-independent maximum egg-to-smolt survival
...
$$
\begin{aligned}
M_{jt} &= \widehat{M}_{jt} \, \text{exp}( \eta^\text{year}_{M,t} + \epsilon_{M,jt} )
\\
...
[\text{log}(p_{3jt}/p_{5jt}), \text{log}(p_{4jt}/p_{5jt})] ^ \top
$$
