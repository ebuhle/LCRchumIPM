...
This would be the place to explain the groups of chum we are modeling. They would be most accurately and historically called stocks, but **salmonIPM** is organized around "populations" so we use the terms interchangeably. In this case it's confusing because the TRT has identified ostensibly independent populations for recovery planning. I propose we change to using primarily "stock" in this document, and explain that they are likely demographically nonindependent subpopulations, but that the **salmonIPM** notation uses $^\text{pop}$ and `fish_data$pop` to index the groups characterized by, e.g., different S-R parameters. We can continue to call the TRT groups "recovery populations", or perhaps "TRT populations"? 
...
As with the smolt recruitment process error, the SAR process model can accommodate covariates $\mathbf{x}_{MSjt}$ with coefficients $\boldsymbol{\beta}_{MS}$. We use this option below to model the difference between SAR of natural and hatchery populations.
...
To complete the model specification, we need priors on all top-level or hyperparameters as well as initial states that are not generated by the process model. In the table below, the power-exponential (also known as generalized normal or Subbotin) density
...
print(fit_Ricker, prob = c(0.05, 0.5, 0.95),
      pars = c("psi","Mmax","eta_year_M","eta_year_MS","eta_pop_p","mu_pop_alr_p","p",
               "p_F","P_D","tau_M","tau_S","B_rate","E_hat","M","S","s_MS",
               "q","q_F","q_O","p_HOS","LL"), 
