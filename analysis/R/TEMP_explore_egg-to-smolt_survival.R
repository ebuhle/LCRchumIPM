...
# NB: This assumes egg production is density-independent
...
psi2 <- array(NA, dim(E))
for(i in levels(pop)) {
  psi1[,pop==i][,-sum(pop==i)] <- E[,pop==i][,-sum(pop==i)] / M[,pop==i][,-1]
...
data.frame(pop = pop, year = year, psi1_l = colQuantiles(psi1, probs = 0.05), 
  ...
  ggplot(aes(x = year, y = psi1_m, ymin = psi1_l, ymax = psi1_u)) +
  geom_ribbon(fill = "darkblue", alpha = 0.5) +
  geom_line(lwd = 1, col = "darkblue") + 
  geom_line(aes(x = x, y = y), data = data.frame(x = range(year), y = c(1,1)), inherit.aes = FALSE) +
  scale_y_log10() + facet_wrap(vars(pop), ncol = 4) + theme_bw()
...
# Not really
par(mar = c(8,4,1,1))
boxplot(split(zeta_M, pop), las = 2)
abline(h = 0)
...
# clean up
rm(list = c('mod','S','mu_E','q','M','S_obs','M_obs','f','E','dat','pop','year',
